...
현재는 read-only 차량 데이터 경로를 우선 개발합니다. opendbc 기반 Genesis G80 DBC subset에서
...
1. **G80 DBC catalog 확장** — 도어·안전벨트·와이퍼·공조·진단 신호를 raw diagnostics로
   추가하고, source revision과 라이선스 provenance를 고정합니다.
2. **Diagnostics HMI** — raw DBC signal의 최신값·관측 시각·bus를 HMI Diagnostics에서
   조회할 수 있게 합니다.
3. **실차 신호 승인** — Canable 수신 로그로 G80 연식·시장·트림을 확인하고 enum 의미를
